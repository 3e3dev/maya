...
import { BASE, type Site, stories, featuredNames, titleFor, artFor, rankName, geojson, findSites } from '@/lib/atlas';
...
export default function Atlas() {
 ...
 const visible=useMemo(()=>query||all ? matches.slice(0,limit) : featuredNames.map(name=>sites.find(s=>s.name===name)).filter((s):s is Site=>!!s),[sites,matches,query,all,limit]);
 ...
 const story=selected?stories[selected.name]:null;
 return <main className="atlas">
  ...
  <aside className={`explorer ${mobileExplore?'expanded':''} ${selected?'hidden-panel':''}`} aria-label="Explore archaeological sites">
   ...
   <div className="catalogue-header"><span>{query?`${matches.length.toLocaleString()} RESULTS`:all?'ALL SITE RECORDS':'START EXPLORING'}</span><button onClick={()=>{setAll(!all);setQuery('');setLimit(60);setMobileExplore(true);}}>{all?'Highlights':'View all'} <ArrowUpRight size={14}/></button></div>
  ...
  <Sheet modal={false} open={!!selected} onOpenChange={(open,details)=>{if(!open&&details.reason!=='outside-press')closeSite();}}>
   <SheetContent side="left" className="site-sheet" showCloseButton={false}>
    {selected&&<div key={selected.id} className="detail-inner">
     ...
     <p className="site-description">{story?.description||`${selected.name} is recorded in the Electronic Atlas of Ancient Maya Sites in ${selected.region !== 'Unspecified'?`${selected.region}, `:''}${selected.country !== 'Unspecified'?selected.country:'an unspecified country'}. The survey classifies this record as ${rankName(selected.rank).toLowerCase()}. This entry preserves the available location and survey information; a detailed historical account is not included in the source dataset.`}</p>
