  ...
  <Sheet modal={false} open={!!selected&&!covered} onOpenChange={(open,details)=>{if(!open&&details.reason!=='outside-press')closeSite();}}>
   <SheetContent side="left" className="site-sheet" showCloseButton={false}>
    {selected&&<div key={selected.id} className="detail-inner">
     ...
     <figure className="site-art"><Image width={1536} height={1024} unoptimized src={`${BASE}/art/${artFor(selected)}.webp`} alt={illustration?`Interpretive drawing: ${illustration.title}`:story?`Interpretive ink and watercolor drawing of ${story.title}`:'Conceptual drawing of a regional archaeological landscape'}/><figcaption>{illustration?'DRAWN FROM AN INAH PHOTOGRAPH':story?'INTERPRETIVE SITE DRAWING':'REGIONAL STUDY · CONCEPTUAL LANDSCAPE'}</figcaption></figure>
