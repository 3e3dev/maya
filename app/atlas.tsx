'use client';
import { landscapeFor } from '@/lib/landscapes';
...
import { BASE, type Site, type SiteDetail, referenceArt, surveyNotesFor, stories, featuredIds, titleFor, artFor, rankName, geojson, findSites } from '@/lib/atlas';
...
export default function Atlas({covered=false}:{covered?:boolean}) {
 ...
 const illustration=selected?referenceArt[selected.id]:null;
 const historySources: import('@/lib/atlas').Source[]=selectedDetail?.sources|| (story?[{url:story.source,label:story.sourceLabel}]:[]);
  ...
  <Sheet modal={false} open={!!selected&&!covered} onOpenChange={(open,details)=>{if(!open&&details.reason!=='outside-press')closeSite();}}>
   <SheetContent side="left" className="site-sheet" showCloseButton={false}>
    {selected&&<div key={selected.id} className="detail-inner">
     ...
     <figure className="site-art"><Image width={1536} height={1024} unoptimized src={`${BASE}/art/${artFor(selected)}.webp`} alt={illustration?`Interpretive drawing: ${illustration.title}`:story?`Interpretive ink and watercolor drawing of ${story.title}`:`Conceptual landscape: ${landscapeFor(selected.id).title}`}/><figcaption>{illustration?'DRAWN FROM AN INAH PHOTOGRAPH':story?'INTERPRETIVE SITE DRAWING':'CONCEPTUAL LANDSCAPE · SHARED ILLUSTRATION'}</figcaption></figure>
     ...
     {selected.notes&&<details><summary>Survey notes</summary><p>{surveyNotesFor(selected)}</p></details>}
     <details className="source-details"><summary>Sources & illustration</summary><p>Coordinates: <a href="https://mayamap.org" target="_blank" rel="noreferrer">MayaMap / EAAMS</a>, compiled by Walter Witschey and Clifford Brown. WGS84; survey positions may be approximate.</p>{selected.reference&&<p>{selected.reference}</p>}{historySources.map(source=><p key={source.url}>History: <a href={source.url} target="_blank" rel="noreferrer">{source.label} <ArrowUpRight size={12}/></a>{source.author&&<> · {source.author}</>}{source.credit&&<>. {source.credit}. English summary.</>}</p>)}<p>Catalogue review: {reviewSummary.reviewedAt}. Survey ranks describe the original inventory, not present condition or cultural importance.</p>{illustration&&<p>Photographic reference: <a href={illustration.sourceUrl} target="_blank" rel="noreferrer">{illustration.title}</a>. {illustration.credit}. {illustration.date}. The drawing follows the historical view and may differ from the site today. Source photograph: {illustration.rights}.</p>}<p>{story||illustration?'AI-generated interpretive artwork; architectural details are illustrative.':'One of 20 AI-generated conceptual landscapes, randomly assigned and shared across atlas entries. This is not a depiction or reconstruction of this specific site.'}</p></details>
