...
const highlights = {};
const labelFor = url => url.includes('unesco.org')?'UNESCO World Heritage Centre':url.includes('inah.gob.mx')?'Instituto Nacional de Antropología e Historia':'Wikipedia · archaeological overview';
...
function surveyNote(s){
 ...
 if(/\bmounds?\b|\bplatform\b|\bmonument|\bvault|\bstructure\b|\bpalace\b|\btemple\b|\bsized?\b|\b\d+\s*[xX]\s*\d+\s*m\b/i.test(n)&&!n.startsWith('rank'))return `The survey records: “${n}”. This brief field annotation is not a complete account of the settlement.`;
...
for(const s of sites){
 const h=highlights[s.id],p=profiles[s.id];
 const candidates=index.filter(a=>a.candidates?.includes(s.id));
 const state=h?'illustrated-profile':p?'sourced-profile':candidates.length?'unresolved-reference-match':'survey-record';
 ...
 if(identityCounts.get(JSON.stringify([s.name,s.coordinates]))>1&&!h&&!p)description+=` This is inventory record ${s.id}; another source entry has the same name and coordinate, and both are preserved separately.`;
 const detailSources=h?[{url:h.source,label:h.sourceLabel}]:p?.sources||[];
 details[s.id]={description,sources:detailSources,reviewStatus:state,...(nearest?{nearbyHighlight:nearest.id,straightLineKm:Math.round(nearest.km*10)/10}:{})};
 audit.push({id:s.id,status:state,notesReviewed:!!s.notes,referencePresent:!!s.reference,candidateUrls:candidates.map(a=>a.url)});
}
assert.equal(Object.keys(details).length,5223);
const summary={reviewedAt:'2026-09-06',records:sites.length,highlights:50,sourcedProfiles:Object.keys(profiles).length+50,surveyProfiles:sites.length-Object.keys(profiles).length-50,sourceIndexUrl:'https://en.wikipedia.org/wiki/List_of_Maya_sites',sourceIndexEntries:index.length,retrievedIndexEntries:index.filter(a=>a.retrieved).length,recordsWithOriginalNotes:sites.filter(s=>s.notes).length,method:'All original names, locations, ranks, notes and bibliographic fields were reviewed through a catalogue-wide source-index comparison. Name matches were checked against coordinates and regional context before attaching an original summary. Additional official UNESCO and INAH references and explicit aliases were reviewed for selected entries. Unresolved names, modern-town articles and conflicting locations were not automatically assigned histories. This is a bounded catalogue review, not an exhaustive search of all archaeological publications. Survey-only descriptions use source fields and calculated straight-line proximity, never inferred chronology or political affiliation.',coordinatePolicy:'All 5,223 original records and their full-precision coordinates are unchanged. Historical ranks are inventory categories, not ratings of cultural importance or present condition.',originalSitesSha256:createHash('sha256').update(await readFile('public/data/sites.json')).digest('hex')};
